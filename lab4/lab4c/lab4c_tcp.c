...
char usage[] = "./lab4c_tcp : collect and print the current temperature\n"
  "Options: \n"
  ...
  "--id=NUM         specify the id of the user\n"
  "--host=hostname  specify the host name of the server\n"
  "--log=logfile    specify the name of the logfile for the program\n"
  "PORTNUM          non-switch option, specify portnumber for server\n";
...
// for 4c
int id_flag = 0; //flags for each required option
int host_flag = 0;
int port_flag = 0;
...
int main(int argc, char** argv) {
  ...
  while ( (opt =  getopt_long(argc, argv, "s:p:l:i:h:", longopts, NULL)) != -1) {
    ...
    case 'l': //log
      ...
      break;
    case 'i': //id
      if (strlen(optarg) != 9) { //if too short/long
      ...
      break;
    case 'h':
      if (strlen(optarg) > 1023) { //if host name too long (1024 limit)
  ...
  if (optind < argc) //report extra arguments
  {
    ...
    for (i = optind; i < argc; i++) {
        ...
        else {//get port number for first non-switch param
          portno = atoi(argv[i]);
          port_flag = 1;
        }
    }

    if ((i-optind) > 1) { //too many non switch params
      fprintf(stderr, "%s", usage);
      exit_proc(1);
    }
  }

  //if we have required parameters that were not passed
...
void setup_tcp() { //socket set up from source https://aticleworld.com/socket-programming-in-c-using-tcpip/
  ...
  char id_msg[14];
  sprintf(id_msg, "ID=%s\n", id); //write to buffer
  write(1, id_msg, 13); //write to server
  return;
}
